-- ═══════════════════════════════════════════════════════════════════════════
-- Eslabón cheque ↔ pago · sobre la versión del repo
--
...
-- ── Qué cierra ─────────────────────────────────────────────────────────────
--
-- El hueco del bloque 8: cobrar con cheque cambiaba la cuenta del debe a
-- VALORES_A_DEPOSITAR y NO dejaba registro de qué cheque era. Cero filas en
-- `cheque`, cero `insert into cheque` en todo el repo. Por eso
-- cambiar_estado_cheque() recibía un p_cheque_id de una fila que nadie creaba,
-- y por eso un cheque rechazado no sabía qué cuota reabrir.
--
-- ── Por qué esta migración y no la de la rama ──────────────────────────────
--
-- Horacio resolvió esto en `feat/eslabon-cheque-pago` y lo resolvió bien: el
-- vínculo, la validación y el alta son suyos y entran tal cual. Pero escribió
-- registrar_cobro sobre la versión de PRODUCCIÓN, y el repo tenía otra —mejor—
-- que nunca se había aplicado. Aplicar la suya habría perdido, en silencio:
--
--   · la validación final `v_agrupado <> v_imputado` con su mensaje
--     «El asiento cubriría % de los % imputados: alguna cuota no resolvió su
--     concepto», que dice QUÉ pasó en vez de sólo que no cuadra;
...
--
-- Las dos validaciones son EQUIVALENTES —antes de ese punto ya se validó
-- `v_imputado = p_monto`— así que esto no cambia qué acepta la función. Cambia
-- qué dice cuando falla, y qué sabe quien la lea en seis meses.
--
-- Así que esta migración parte de la versión del repo (20260805102032) y le
-- agrega SÓLO los cuatro injertos del eslabón. De paso cierra la divergencia
-- repo↔producción que quedó documentada en decisiones.md, hacia el lado bueno.
--
-- ── Lo que NO se toca ──────────────────────────────────────────────────────
--
-- El `case` de la caja —`when 'cheque' then 'VALORES_A_DEPOSITAR'`, decisión
-- 31—, el orden de las validaciones, crear_asiento y el coalesce del
-- responsable quedan exactamente como estaban.
--
-- ── Compatibilidad ─────────────────────────────────────────────────────────
--
-- Los tres parámetros van AL FINAL y con default null, y las dos ramas nuevas
-- están dentro de `if p_medio = 'cheque'`: un cobro sin cheque recorre el mismo
-- código de siempre. El front llama por nombre. Los pagos ya registrados no se
-- tocan — esto sólo redefine la función.
-- ═══════════════════════════════════════════════════════════════════════════


-- El vínculo que faltaba. `if not exists` porque la rama de Horacio puede
-- haberla agregado ya en otro entorno.
alter table cheque add column if not exists pago_id uuid references pago(id);

comment on column cheque.pago_id is
  'El pago (cobro) que originó este cheque recibido. Es el eslabón que permite, '
  'cuando el cheque rebota, llegar a las cuotas que hay que reabrir. Nulo en '
  'cheques emitidos hasta que se resuelva el mismo eslabón para gastos.';


-- La firma cambia (3 params nuevos), así que sin el drop quedaría una
-- sobrecarga de 7 argumentos conviviendo con la de 10 y las llamadas serían
-- ambiguas. Apunta a la firma que hoy tiene producción.
drop function if exists registrar_cobro(uuid, numeric, text, date, jsonb, uuid, uuid);
...
comment on function registrar_cobro(uuid, numeric, text, date, jsonb, uuid, uuid, text, text, date) is
  ...
  'en `cheque` vinculada por pago_id — el eslabón del bloque 8.';
