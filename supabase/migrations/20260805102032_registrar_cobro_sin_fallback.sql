-- ============================================================================
-- CAMPA · registrar_cobro · sacar el fallback de usuario
--
-- Reemplaza SOLO la resolución del responsable. El resto de la función queda
-- byte a byte como estaba en 20260731100343_registrar_cobro.sql.
--
-- ── El problema ─────────────────────────────────────────────────────────────
--
-- La línea era:
...
--
-- y hacía dos daños distintos:
--
--   1. ROMPE DESDE EL FRONT. Sin sesión, auth.uid() es null y la subconsulta se
--      evalúa. El rol no puede leer auth.users, así que el error que llega es
--      "permission denied for table users" — que no dice nada de lo que pasó.
--      Con sesión válida esto NUNCA se evaluaba: coalesce corta en el primer
--      argumento no nulo, así que el camino feliz no cambia al sacarlo.
--
--   2. REGISTRA UN RESPONSABLE FALSO, que es lo más serio. Desde un rol que SÍ
--      puede leer auth.users —el servidor— un cobro sin p_responsable_id
--      quedaba atribuido AL PRIMER USUARIO DE LA TABLA, en silencio. Eso no
--      falla: miente sobre quién registró el cobro, y queda escrito en
--      pago.registrado_por y en asiento.created_by.
--
-- ── Lo que NO se hace ───────────────────────────────────────────────────────
--
-- NO se usa SECURITY DEFINER. Reemplaza a la propuesta de la rama
-- fix/registrar-cobro-definer (20260802160000), que resolvía el síntoma —el
-- permiso sobre auth.users— dejando vivo el problema de fondo: con DEFINER el
-- fallback funciona siempre, así que el responsable falso pasaría a ser el
-- comportamiento normal en vez de un error. Y correr la función con los
-- privilegios del dueño para poder leer una tabla que no hace falta leer
-- agranda la superficie sin necesidad.
--
-- ── Pendiente, fuera de alcance ─────────────────────────────────────────────
--
-- crear_asiento() tiene EL MISMO fallback, y lo llaman nueve funciones que le
-- pasan p_created_by => null (los devengos de socios y sponsors, los retiros,
-- la compra y venta de USD, la entrega a central, anular_asiento). Acá no
-- molesta porque registrar_cobro le pasa v_user_id explícito y el coalesce
-- corta antes. Se trata aparte.
-- ============================================================================
