...
/** Mes y año corto para el eje X: "mar 26". Parseado en UTC para no correrse de mes. */
function formatMesCorto(mes: string): string {
  const match = SOLO_FECHA.exec(mes)
  if (!match) return mes
  const [, aaaa, mm] = match
  const fecha = new Date(Date.UTC(Number(aaaa), Number(mm) - 1, 1))
  const mesTxt = new Intl.DateTimeFormat('es-AR', { month: 'short', timeZone: 'UTC' }).format(
    fecha
  )
  return `${mesTxt} ${aaaa.slice(2)}`
}
...
export default async function ProyeccionMensualPage() {
  const supabase = await createClient()

  const { data, error } = (await supabase
    .from('v_cashflow_mensual' as never)
    .select('*')
    .order('mes')) as unknown as {
    data: FilaCashflowMensual[] | null
  ...
  return (
    <main className="p-8 font-sans">
      <h1 className="text-2xl font-bold mb-1">Proyección de caja — 12 meses</h1>
      <p className="text-sm text-gray-500 mb-6">
        Saldo mensual proyectado — real hasta el mes en curso, estimado hacia adelante.
      </p>
        ...
        <>
            ...
            <div className="border border-gray-200 rounded p-4">
              ...
              {filaQuiebre ? (
                <div className="text-lg font-bold text-red-600">
                  {filaQuiebre.mes ? formatDate(filaQuiebre.mes) : 'Mes desconocido'}
                </div>
              ) : (
          ...
          <div className="border border-gray-200 rounded p-4 mb-6">
            <svg viewBox={`0 0 ${ANCHO} ${ALTO}`} className="w-full h-80">
              ...
              {/* Eje X: etiquetas (todos los meses, son pocos) */}
              {filas.map((f, i) => (
                <text
                  key={i}
                  x={xScale(i)}
            ...
            <tbody>
              {filas.map((fila, i) => {
                ...
                return (
                  ...
                  >
                    <td className="py-2 pr-4">{formatDate(fila.mes)}</td>
